...
class KleLayoutParserTest extends AnyFlatSpec with Matchers {
  ...
  it should "parse yaml" in {
    val actual = YamlMapper.readValue[Layout](asYamlString(
  ...
  it should "correct number of columns" in {
    YamlMapper.readValue[Layout](asYamlString(
      """[ "", "", "", "", "", "", "" ]"""
    )).rows.head.keys.length should be(7)

    YamlMapper.readValue[Layout](asYamlString(
      """[ "A", "B" ]"""
    )).rows.head.keys.length should be(2)
  ...
  it should "read correct width and position" in {
    YamlMapper.readValue[Layout](asYamlString(
      """[ "", "", "", "", "", "", "" ]"""
    )).rows.head.keys.last.position.x should be(6)

    YamlMapper.readValue[Layout](asYamlString(
      """[ "", {x:3.75, w:0.25}, "", "" ]"""
    )).rows.head.keys.last.position.x should be(5)
  ...
  it should "read complete keyboard" in {
    YamlMapper.readValue[Layout](asYamlString(
      """[ "", {x:2}, "" ],
        |[ {x:1,w:2}, "" ],
        |[ "", {x:0.5}, "", {x:0.5}, "" ],
      ...
      Layout(
        Row( Key(), Key(x = 3) ),
        Row( Key(x = 1, w = 2) ),
        Row( Key(), Key(x = 1.5), Key(x = 3) ),
        Row( Key(x = 0.75), Key(x = 2.25) )
      )
    )
  }
